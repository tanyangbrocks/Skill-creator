...
*最後更新：2026-06-09（Contact 容器、InvokeSpell 連段、存讀檔、敵人重生、命中位置確認）*

---

## 目前狀態：Phase 1 ✅　Phase 2 ✅　角色整合 ✅　全內容填入 ✅　VM 接通 ✅　積木編輯器 ✅　敵人 AI ✅　技能槽/容器雛形 ✅　Contact/連段/存檔 ✅　敵人重生 ✅　命中位置 ✅
...
### Phase 3 核心
- 程序生成地圖、NPC 系統、合成系統、採集與建築
- 完整 VM Phase 2（Vector 型別、List 積木、廣播通訊、實體查詢）
- 完整 Scratch 式拖拉積木編輯器（取代現有列表式）
- 敵人多樣化 AI（巡邏、遠距攻擊、法術施放）
...
| 敵人重生時間 8 秒 | `Enemy.cs RespawnTime` | ⚠️ 待平衡調整 |
| Contact 直接傷害 20 | `SpellCaster.ExecuteContactHit` | ⚠️ 待平衡調整 |
